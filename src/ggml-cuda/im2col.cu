...
template <typename src_t>
static void im2col_dispatch_dst(const src_t * src1_d, ggml_tensor * dst,
    int64_t IW, int64_t IH, int64_t OW, int64_t OH, int64_t KW, int64_t KH, int64_t IC,
    int64_t N, int64_t IC_IH_IW, int64_t IH_IW,
    int s0, int s1, int p0, int p1, int d0, int d1, cudaStream_t stream) {
    switch (dst->type) {
        case GGML_TYPE_F16:
            im2col_cuda<src_t, half>(src1_d, (half *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
            return;
        case GGML_TYPE_F32:
            im2col_cuda<src_t, float>(src1_d, (float *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
            return;
        case GGML_TYPE_BF16:
            im2col_cuda<src_t, nv_bfloat16>(src1_d, (nv_bfloat16 *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
            return;
        default:
...
void ggml_cuda_op_im2col(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    switch (src1->type) {
        case GGML_TYPE_F32:
            im2col_dispatch_dst<float>((const float *)src1->data, dst, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
            return;
        case GGML_TYPE_F16:
            im2col_dispatch_dst<half>((const half *)src1->data, dst, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
            return;
        case GGML_TYPE_BF16:
            im2col_dispatch_dst<nv_bfloat16>((const nv_bfloat16 *)src1->data, dst, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
            return;
        default:
