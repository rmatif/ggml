...
__device__ __forceinline__ float im2col_load_f32(const src_t * x, int64_t idx) {
    return ggml_cuda_cast<float>(x[idx]);
}

template <typename dst_t>
__device__ __forceinline__ dst_t im2col_store(float v) {
    return ggml_cuda_cast<dst_t>(v);
}

template <typename src_t, typename dst_t>
static  __global__ void im2col_kernel(
        const src_t * x, dst_t * dst,
    ...
    for (int64_t iow = blockIdx.y; iow < OW; iow += gridDim.y) {
        for (int64_t iz = blockIdx.z; iz < N_OH; iz += gridDim.z) {
            ...
            if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
                dst[offset_dst] = im2col_store<dst_t>(0.0f);
            } else {
                const int64_t offset_src = iic * IC_IH_IW + in * IH_IW;
                dst[offset_dst] = im2col_store<dst_t>(im2col_load_f32(x, offset_src + iih * IW + iiw));
            }
        }
    }
...
template <typename src_t, typename dst_t>
static void im2col_cuda(const src_t * x, dst_t* dst,
    ...
    im2col_kernel<src_t, dst_t><<<block_nums, MIN(IC_KH_KW, CUDA_IM2COL_BLOCK_SIZE) , 0, stream>>>(x, dst, IC, IW, IH, OH, OW, KW, KH,
...
void ggml_cuda_op_im2col(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const int64_t src1_type_size = ggml_type_size(src1->type);
    const int64_t IC_IH_IW = src1->nb[is_2D ? 2 : 1] / src1_type_size; // nb is byte offset
    const int64_t N        = src1->ne[is_2D ? 3 : 2];
    const int64_t IH_IW    = src1->nb[is_2D ? 3 : 2] / src1_type_size; // nb is byte offset

    if (src1->type == GGML_TYPE_F32) {
        const float * src1_d = (const float *)src1->data;
        if (dst->type == GGML_TYPE_F16) {
            im2col_cuda<float, half>(src1_d, (half *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else if (dst->type == GGML_TYPE_F32) {
            im2col_cuda<float, float>(src1_d, (float *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else if (dst->type == GGML_TYPE_BF16) {
            im2col_cuda<float, nv_bfloat16>(src1_d, (nv_bfloat16 *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else {
            GGML_ABORT("im2col: unsupported dst type");
        }
    } else if (src1->type == GGML_TYPE_F16) {
        const half * src1_d = (const half *)src1->data;
        if (dst->type == GGML_TYPE_F16) {
            im2col_cuda<half, half>(src1_d, (half *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else if (dst->type == GGML_TYPE_F32) {
            im2col_cuda<half, float>(src1_d, (float *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else if (dst->type == GGML_TYPE_BF16) {
            im2col_cuda<half, nv_bfloat16>(src1_d, (nv_bfloat16 *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else {
            GGML_ABORT("im2col: unsupported dst type");
        }
        ...
        if (dst->type == GGML_TYPE_F16) {
            im2col_cuda<nv_bfloat16, half>(src1_d, (half *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else if (dst->type == GGML_TYPE_F32) {
            im2col_cuda<nv_bfloat16, float>(src1_d, (float *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else if (dst->type == GGML_TYPE_BF16) {
            im2col_cuda<nv_bfloat16, nv_bfloat16>(src1_d, (nv_bfloat16 *) dst->data, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else {
            GGML_ABORT("im2col: unsupported dst type");
        }
    } else {
        GGML_ABORT("im2col: unsupported src type");
    }
}
