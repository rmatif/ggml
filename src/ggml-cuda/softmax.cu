...
// ...existing code...
template <bool use_shared, int ncols_template, int block_size_template, typename X, typename T>
static __global__ void soft_max_f16(
        const X * x, const T * mask, const float * sinks, X * dst, const soft_max_params p) {
...
#pragma unroll
    for (int col0 = 0; col0 < ncols; col0 += block_size) {
        ...
        // convert input half (or other X) to float before scaling
        const float xv = t2f32(x[col]);
        const float val = xv * p.scale + (mask ? slope * t2f32(mask[col]) : 0.0f);
...
#pragma unroll
    for (int col0 = 0; col0 < ncols; col0 += block_size) {
        ...
        dst[col] = ggml_cuda_cast<X>(vals[col] * inv_sum);
    }
}
// ...existing code...

template<int... Ns, typename X, typename T>
static void launch_soft_max_kernels_f16(const X * x, const T * mask, const float * sinks, X * dst,
                             const soft_max_params & p, cudaStream_t stream, dim3 block_dims, dim3 block_nums, size_t nbytes_shared)
{
    ...
    auto launch_kernel = [=](auto I) -> bool {
        ...
        if (p.ncols == ncols) {
            CUDA_SET_SHARED_MEMORY_LIMIT((soft_max_f16<true, ncols, block, X, T>), smpbo);
            soft_max_f16<true, ncols, block, X, T><<<block_nums, block_dims, nbytes_shared, stream>>>
                (x, mask, sinks, dst, p);
            return true;
        }
        return false;
    ...
    //default case
    CUDA_SET_SHARED_MEMORY_LIMIT((soft_max_f16<true, 0, 0, X, T>), smpbo);
    soft_max_f16<true, 0, 0, X, T><<<block_nums, block_dims, nbytes_shared, stream>>>(x, mask, sinks, dst, p);
}

template<typename T>
static void soft_max_f16_cuda(const half * x, const T * mask, const float * sinks, half * dst, const soft_max_params & params, cudaStream_t stream) {
    int nth = WARP_SIZE;
    ...
    if (nbytes_shared <= smpbo) {
        launch_soft_max_kernels_f16<32, 64, 128, 256, 512, 1024, 2048, 4096>(x, mask, sinks, dst, params, stream, block_dims, block_nums, nbytes_shared);
    } else {
        // const size_t nbytes_shared_low = WARP_SIZE*sizeof(float);
        // soft_max_f16<false, 0, 0, half, T><<<block_nums, block_dims, nbytes_shared_low, stream>>>(x, mask, sinks, dst, params);
        GGML_ASSERT(false);
    }
}
...
void ggml_cuda_op_soft_max(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (use_f16) {
        // src0 is F32, mask is F16
        soft_max_f32_cuda(src0_d, (const half  *) src1_d, (const float *) src2_d, dst_d, params, stream);
    } else if (src0->type == GGML_TYPE_F16) {
        // src0 is F16
        if (src1 && src1->type == GGML_TYPE_F16) {
            soft_max_f16_cuda((const half *) src0_d, (const half *) src1_d, (const float *) src2_d, (half *) dst_d, params, stream);
        } else { 
            soft_max_f16_cuda((const half *) src0_d, (const float *) src1_d, (const float *) src2_d, (half *) dst_d, params, stream);
        }
    } else {
        // both src0 and mask are float
        soft_max_f32_cuda(src0_d, (const float *) src1_d, (const float *) src2_d, dst_d, params, stream);
    }
}
